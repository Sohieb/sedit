...
typedef struct erow {
    ...
    char *chars;
    char *render;
    unsigned char *hl;                      // the row highlight
} erow;
...
int editorRowCxToRx(erow *row, int cx) {
    ...
    for (j = 0; j < cx; ++j) {
        if (row->chars[j] == '\t')
            rx += (SEDIT_TAB_STOP - 1) - (rx % SEDIT_TAB_STOP);
        rx++;
    }
    return rx;
}
...
int editorRowRxToCx(erow *row, int rx) {
    ...
    for (cx = 0; cx < row->size; ++cx) {
        if (row->chars[cx] == '\t') 
            cur_rx += (SEDIT_TAB_STOP - 1) - (cur_rx % SEDIT_TAB_STOP);
        cur_rx++;
...
void editorUpdateRow(erow *row) {
    ...
    free(row->render);
    row->render = malloc(row->size + tabs * (SEDIT_TAB_STOP - 1) + 1);

    int idx = 0;
    for (j = 0; j < row->size; ++j) {
        if (row->chars[j] == '\t') {
            row->render[idx++] = ' ';
            while (idx % SEDIT_TAB_STOP != 0) row->render[idx++] = ' ';
        } else {
...
void editorSave() {
    ...
    free(buf);
    editorSetStatusMessage("Can not save! T/O erroe %s", strerror(errno));
}
    ...
    for (y = 0; y < E.screenrows; ++y) {
        ...
        if (filerow >= E.numrows) {
            if (E.numrows == 0 && y == E.screenrows / 3) {
                ...
                "sedit text editor -- version %s", SEDIT_VERSION);
...
void initEditor() {
    ...
    E.screenrows -= 2;          // decrease by two lines to make space for the status bar
}
